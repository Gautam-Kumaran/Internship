...
cv2.imwrite('Outline_image.png', thresh)
...
area = 78514
beginning_distance = ((4*area)/math.pi)**(1/2)
count = np.zeros(len(clockwisemovement))

for i in range(len(clockwisemovement)):
    currentpoint =  clockwisemovement[i]
    for j in range(len(clockwisemovement)):
        if (distance_calculation(currentpoint,clockwisemovement[j])>beginning_distance):
            currentpoint = clockwisemovement[j]
            count[i] = count[i] + 1
